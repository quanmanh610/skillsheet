...
const AddProjectSkillCategory = ({ onAddExistCategory, onAddNewCategory, onCancel, useableCategories }) => {
  ...
  const validateSkillCategory = (rule, value) => {
    if (value !== undefined && value !== "" && value.trim() === "") {
      return Promise.reject('Category cannot contain only white space characters.');
    } else if (value.length > 100) {
      return Promise.reject(Messages.SKILL_CATEGORY_VALIDATE_MAX_LENGTH);
    } else {
          ...
          >
            <Form.Item
              ...
              rules={[
                ...
                {
                  validator: validateSkillCategory
                }
              ]}
...
export default AddProjectSkillCategory;
