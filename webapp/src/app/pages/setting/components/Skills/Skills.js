...
import { deleteSkill, activeAllSkill, checkSkill } from '../../../../api/setting';
...
const Skills = React.forwardRef((props, ref) => {
  ...
  const dataSource = useSelector(getSkillsInStore);
  const dataSourceBK = useSelector(getSkillsBKInStore);
  ...
  const rowSelection = {
    ...
    onChange: (selectedRowKeys, selectedRows) => {
      setSelectedDeleteSkills(selectedRows);
  ...
  const onDeleteAll = async () => {
    ...
    const count = checkSkill(body);
    count.then(async function(result) {
      if(result.data > 0) {
        setdeleteConfirmVisible(true);
        setProfileSkill(body);
      } else {
        const response = await deleteSkill(body);
        if (response.status === HttpStatus.OK) {
          ...
          openNotification(
            Messages.ERROR,
            Messages.deleteUnsuccessMessage('Skill'),
            ''
          );
        }
      }
  ...
  const onActiveAll = async () => {
    const inActives = selectedDeleteSkills.filter(
      (item) => item.status === SettingItemStatus.INACTIVE
    );
    const deleteSkillId = {
      idSelectedLst: [...selectedRowKeys],
    ...
    if (inActives.length !== 0) {
      const response = await activeAllSkill(deleteSkillId);
      if (response) {
        setSelectedRowKeys([]);
        await dispatch(fetchSkills());
        openNotification(
          Messages.SUCCESS,
          Messages.activeAllSuccessMessage('Skill'),
          ''
        );
      } else {
        openNotification(
          Messages.ERROR,
          Messages.activeAllUnsuccessMessage('Skill'),
          ''
        );
      }
    }
    ...
    },
    {
      ...
      render: (record) => {
        ...
        const onDelete = async () => {
          clearRowSelection();
          const deleteSkillIdList = {
            idSelectedLst: [record.skillId],
          };
          const count = checkSkill(deleteSkillIdList);
          count.then(async function(result) {
            if(result.data > 0) {
              setdeleteConfirmVisible(true);
              setProfileSkill(deleteSkillIdList);
            } else {
              const resp = await deleteSkill(deleteSkillIdList);
              if (resp) {
                // setDeleteEditVisible(false);
                await dispatch(fetchSkills());
                openNotification(Messages.SUCCESS, Messages.DELETE_SKILL_SUCCESS, '');
              } else {
                openNotification(Messages.ERROR, Messages.DELETE_SKILL_UNSUCCESS, '');
              }
            }
          });
        };
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    ...
    if (sorter.order) {
      dispatch(sortSkills(sorted));
    } else {
      dispatch(sortSkills(dataSourceBK));
    }
  };
  ...
  return (
    <div>
      <Row>
        <Col span={24}>
          <SkillHeader
            selectedDeleteSkill={selectedRowKeys}
      ...
      <Row>
        <Col span={24}>
          {/* <div>{selectedDeleteSkillKey}</div> */}
          {isLoading ? (
            <Loading />
          ) : (
              <Table
