import React, { useState } from 'react';
import { Space, Button, Popconfirm } from 'antd';
import { useDispatch, useSelector } from 'react-redux';
import {
  PlusOutlined,
  CheckCircleOutlined,
  DeleteOutlined,
} from '@ant-design/icons';
import CertificateModal from './CertificateModal';
import { fetchCertificate } from '../../../../../store/slice/settingSlice';
import { addNewCertificate } from '../../../../../api/setting';
import { getUserData } from '../../../../../store/slice/authSlice';
import { Messages } from '../../../../../utils/AppMessage';
import { openNotification } from '../../../../../components/OpenNotification';
import { CSSConstants } from '../../../../../constants/CSSConstants';

const CertificateHeader = ({
  selectedDeleteCertificate,
  onDeleteAll,
  onActiveAll,
  activeAllVisible,
  clearRowSelection,
  certificateList,
}) => {
  const dispatch = useDispatch();
  const [visible, setVisible] = useState(false);
  const userData = useSelector(getUserData);
  const onAddNew = () => {
    setVisible(true);
    clearRowSelection();
  };

  const onSave = async (values) => {
    const newCertificateData = {
      category: null,
      name: null,
      status: null,
      createBy: userData ? userData.userName : 'admin',
    };
    Object.assign(newCertificateData, values);
    setVisible(false);
    await addNewCertificate(newCertificateData);
    await dispatch(fetchCertificate());
    openNotification(
      Messages.SUCCESS,
      Messages.addSuccessMessage('Certificate'),
      ''
    );
  };

  return (
    <Space direction="horizontal">
      <Button
        type="primary"
        icon={<PlusOutlined />}
        onClick={onAddNew}
        size="small"
        style={{ width: CSSConstants.BUTTON_WIDTH * 1.2 }}
      >
        Add New
      </Button>
      {selectedDeleteCertificate.length > 0 ? (
        <div>
          <Popconfirm
            title={Messages.DELETE_CONFIRM}
            onConfirm={onDeleteAll}
            okText="Yes"
            cancelText="No"
          >
            <Button
              size="small"
              type="primary"
              danger
              style={{ width: CSSConstants.BUTTON_WIDTH * 1.2 }}
              icon={<DeleteOutlined />}
            >
              Delete
            </Button>
          </Popconfirm>
          &nbsp;&nbsp;
          {activeAllVisible ? (
            <Popconfirm
              title={Messages.ACTIVE_CONFIRM}
              onConfirm={onActiveAll}
              okText="Yes"
              cancelText="No"
            >
              <Button
                size="small"
                type="primary"
                style={{
                  backgroundColor: CSSConstants.COLOR_GREEN,
                  borderColor: CSSConstants.COLOR_GREEN,
                  width: CSSConstants.BUTTON_WIDTH * 1.2,
                }}
                icon={<CheckCircleOutlined />}
              >
                Activate
              </Button>
            </Popconfirm>
          ) : (
              ''
            )}
        </div>
      ) : (
          ''
        )}
      <CertificateModal
        title="ADD NEW CERTIFICATE"
        visible={visible}
        onSave={onSave}
        onCancel={() => {
          setVisible(false);
        }}
        list={certificateList}
      />
    </Space>
  );
};

export default CertificateHeader;
