...
const EditTemplateProfileModal = ({ visible, onSave, onCancel, title, dataSource, record }) => {
    ...
    const onHandleData = () => {
        ...
        (value) => {
            onSave(record.templateId, selectedFile);
            setHasFile(false);
            form.resetFields();
        }
        ).catch((error) => {
    ...
    const validateFile = (rule, value) => {
        ...
        } else if (isExist == true && record.templateName != name[name.length - 1]) {
    ...
    return (
        ...
        >
        <p>Choose a file to replace {record.templateName}</p>
        <Form
            size="small"
            form={form}
            wrapperCol={{ span: 24 }}
...
export default EditTemplateProfileModal;
